...
async function ytdl(url, format = "mp3") {
  try {
    ...
    if (!videoId) {
      return { status: false, mess: "Format URL tidak dikenali atau bukan link YouTube yang valid." };
    }
    ...
    if (!init?.convertURL) {
      return { status: false, mess: "Gagal menginisialisasi server (Init failed)." };
    }
    ...
    if (!convert?.progressURL || !convert?.downloadURL) {
      return { status: false, mess: "Gagal mendapatkan data konversi." };
    }
...
class Youtube {
  ...
  wrapResponse(data) { return { created_by: this.CREATED_BY, note: this.NOTE, results: data }; }
  async download(url, format = "audio") {
    const outputFormat = ["video", "mp4"].includes(String(format || "audio").toLowerCase()) ? "mp4" : "mp3";
    const result = await ytdl(url, outputFormat);
    if (!result?.status || !result?.dl) throw new Error(result?.mess || "Gagal mengunduh konten YouTube");
    return this.wrapResponse({ title: result.title, download: result.dl, url: result.dl, format: outputFormat });
  }
}
